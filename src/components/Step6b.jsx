...
const Step6b = () => {
  ...
  // Load formData from custom storage
  useEffect(() => {
    const savedData = getStepData('step6b');
  ...
  const handleSubmit = () => {
    if (!agreed) return toast.error('☑️ Please accept the declaration before submitting.');
    if (userCaptcha !== captchaAnswer) return toast.error('❌ Incorrect CAPTCHA. Try again.');

    // Save using custom storage function
    saveStepData('step6b', formData);

    toast.success('✅ Successfully Submitted!');
    setTimeout(() => navigate('/step7'), 1500);
  };
  ...
  const renderInputGroup = (label, name) => (
    <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-4">
      <label className="w-full md:w-60 text-[1.05rem] font-medium text-gray-700">{label}:</label>
      <input
        name={name}
        value={formData[name]}
        onChange={handleChange}
        readOnly={!editable}
        className={`flex-1 border rounded px-4 py-2 text-[1.05rem] ${editable ? 'bg-white' : 'bg-gray-100'}`}
      />
    </div>
  );

  return (
    <div className="w-full md:w-4/5 mx-auto p-6 md:p-10 bg-white shadow-xl rounded-lg font-sans space-y-6">
